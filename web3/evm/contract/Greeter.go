...
// ContractGreeterChangeIterator is returned from FilterGreeterChange and is used to iterate over the raw logs and unpacked data for GreeterChange events raised by the Contract contract.
type ContractGreeterChangeIterator struct {
	Event *ContractGreeterChange // Event containing the contract specifics and raw log
...
func (it *ContractGreeterChangeIterator) Next() bool {
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(ContractGreeterChange)
...
func (it *ContractGreeterChangeIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *ContractGreeterChangeIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}
...
func (_Contract *ContractFilterer) FilterGreeterChange(opts *bind.FilterOpts, greeting []string) (*ContractGreeterChangeIterator, error) {
	...
	for _, greetingItem := range greeting {
		greetingRule = append(greetingRule, greetingItem)
	}

	logs, sub, err := _Contract.contract.FilterLogs(opts, "GreeterChange", greetingRule)
	if err != nil {
		return nil, err
	}
...
func (_Contract *ContractFilterer) WatchGreeterChange(opts *bind.WatchOpts, sink chan<- *ContractGreeterChange, greeting []string) (event.Subscription, error) {
	...
	logs, sub, err := _Contract.contract.WatchLogs(opts, "GreeterChange", greetingRule)
	if err != nil {
		return nil, err
	}

...
import (
	...
	injectx "github.com/Donaldhan/inject"
)
...
func FormatX(name string, company S1, level S2, age int) {
	fmt.Printf("FormatX name ＝ %s, company=%s, level=%s, age ＝ %d!\n", name, company, level, age)
}
func InjectInvokeX() {
	//控制实例的创建
	inj := injectx.New()
	...
	//函数反转调用
	inj.Invoke(Format)
	log.Println("InjectInvokeX Format done==============")
}
...
func InjectInvokeStructX() {
	...
	inj.Apply(&s)
	//打印结果
	fmt.Printf("InjectInvokeStructX:s = %v\n", s)
	log.Println("InjectInvokeStructX done==============")
}
